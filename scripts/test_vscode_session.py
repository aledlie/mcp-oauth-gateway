...
def test_vscode_session_flow():
    ...
    # Initialize session
    response = httpx.post(base_url, json=init_data, headers=headers, timeout=30.0)
    ...
    response = httpx.get(base_url, headers=poll_headers, timeout=30.0)
    ...
    response = httpx.post(base_url, json=tools_data, headers=tools_headers, timeout=30.0)
    print(f"Status: {response.status_code}")
    print(f"Content-Type: {response.headers.get('Content-Type', 'Not specified')}")
    ...
    response = httpx.post(base_url, json=session_info_data, headers=tools_headers, timeout=30.0)
    print(f"Status: {response.status_code}")
    ...
    response = httpx.get(base_url, headers=poll_headers, timeout=5)
    print(f"Poll Status: {response.status_code}")
